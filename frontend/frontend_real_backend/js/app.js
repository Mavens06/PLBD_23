...
// Points CANDIDATS = grille autorisée (3×3), parcourue en SERPENTIN pour un
// trajet propre (colonnes alternées). On exclut le coin de parking (0,0) : le
// robot y démarre, on n'y mesure pas. Les coordonnées sont MASQUÉES à l'usager.
function _gridCandidates() {
  const G = ALLOWED_COORDS;                 // [0, 1.8, 3.6]
  const seq = [];
  G.forEach((x, ci) => {
    const ys = ci % 2 === 0 ? G : [...G].reverse();   // ↑ puis ↓ : pas de saut diagonal
...
// Coche / décoche un point de mesure. Le plan = l'ensemble des points cochés,
// réordonné en serpentin et renuméroté P1..Pn. Aucune coordonnée affichée.
...
function renderPlanEditor() {
  ...
  // Cases à cocher : grille affichée du fond (y haut) vers l'avant (y bas).
  const rowsY = [...ALLOWED_COORDS].reverse();        // 3.6, 1.8, 0
  const cells = rowsY.map((y) => ALLOWED_COORDS.map((x) => {
    if (x === 0 && y === 0) return `<div class="plan-cell" style="visibility:hidden"></div>`;  // parking
...
function applyPlanFromEditor() {
  ...
  // Au moins un point coché. Les points de la grille sont toujours espacés de
  // ≥ MIN_SPACING_M (grille 1,8 m) → pas d'autre validation nécessaire.
