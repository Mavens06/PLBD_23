...
| `DRIVE_THROTTLE` | `-0.25` | Throttle SIGNÉ ligne droite (avant = négatif sur ce câblage, validé au sol). Départ en **échelon direct** (pas de rampe : la rampe anti-brownout faisait dévier le robot à gauche au démarrage) |
...
| `POST_TURN_STRAIGHTEN_S` | `0` | **Redressement post-virage par AVANCE** (au lieu de reculer) : après CHAQUE rotation, courte avance roues droites de N s pour s'assurer que le châssis est bien droit avant de poursuivre/mesurer. « Très légère » par conception (ex. `0.25`). Quand `>0`, **désactive le recul de compensation d'arc** (`TURN_BACKUP_M`) : on ne recule plus, on avance très légèrement à la place. `0` = comportement précédent (recul) |
| `PRE_TURN_BACKUP_M` | `0` | **Recul AVANT chaque rotation** (m PHYSIQUES). Compense un virage en arc « trop grand » : avant de tourner pour rejoindre le point suivant, le robot recule de N m, puis tourne (l'arc le ramène en avant) → il finit ~en place. Appliqué avant chaque leg de type `turn` dans `move_to_point`. `0` = désactivé |
